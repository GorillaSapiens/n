...
static int copy_n_bytes(FILE *in, FILE *out, uint32_t size)
{
   ...
   while (remaining > 0) {
      ...
      if (got != chunk) {
         fprintf(stderr, "n65ar: unexpected end of file while copying payload\n");
         return 0;
      }

      if (fwrite(buffer, 1, got, out) != got) {
         fprintf(stderr, "n65ar: write failed: %s\n", strerror(errno));
         return 0;
      }
...
static int skip_n_bytes(FILE *fp, uint32_t size)
{
   ...
   while (remaining > 0) {
      ...
      if (got != chunk) {
         fprintf(stderr, "n65ar: unexpected end of file while skipping payload\n");
         return 0;
      }
...
static int open_archive_for_read(const char *archive_name, FILE **out_fp)
{
   ...
   if (fread(magic, 1, NAR_MAGIC_SIZE, archive) != NAR_MAGIC_SIZE) {
      fprintf(stderr, "n65ar: '%s' is too short to be a n65ar archive\n", archive_name);
      fclose(archive);
      return 0;
   }

   if (memcmp(magic, NAR_MAGIC, NAR_MAGIC_SIZE) != 0) {
      fprintf(stderr, "n65ar: '%s' is not a n65ar archive\n", archive_name);
      fclose(archive);
      return 0;
   }
